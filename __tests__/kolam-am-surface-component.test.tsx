...
jest.mock('../src/services/am-api', () => ({
  ...
  clearAmWebhookLogs: jest.fn(() => Promise.resolve({deletedCount: 2, configId: null})),
...
describe('KolamAmSurface', () => {
  ...
  it('runs hardware create, edit, and delete actions from the Hardware route', async () => {
    ...
    expect(boxActionMenu).toBeTruthy();
    await act(async () => {
  ...
  it('renders transfer stats and pagination from the Transfers route metadata', async () => {
    ...
    expect(joinedText).not.toContain('Aksi');
    expect(joinedText).toMatch(/999 Vendor Pending\s+BCA/);
    expect(joinedText).toMatch(/888 Vendor Success\s+BRI/);
  ...
  it('runs webhook register, toggle, delete, and test ping actions', async () => {
    ...
    await act(async () => {
      const clearButtons = renderer!.root
        .findAllByType(KolamButton)
        .filter(button => button.props.label === 'Bersihkan Log');
      clearButtons[clearButtons.length - 1].props.onPress();
    });
    ...
    expect(clearAmWebhookLogs).toHaveBeenCalledWith();
    ...
    expect(configText).toContain('2 log webhook dibersihkan');
