...
describe('KolamAppShellSurface', () => {
  ...
  it('renders right rail as an overlay without resizing workspace scroll', async () => {
    let renderer: ReactTestRenderer.ReactTestRenderer;
    ...
    const railOverlay = renderer!.root
      .findAllByType(View)
      .find(
        node => node.props.accessibilityLabel === 'Kolam right rail overlay',
      );

    expect(countMainShellScrollViews(renderer!)).toBe(1);
    expect(railOverlay).toBeDefined();
    expect(StyleSheet.flatten(railOverlay!.props.style)).toEqual(
      expect.objectContaining({
        bottom: 0,
        position: 'absolute',
        right: 0,
