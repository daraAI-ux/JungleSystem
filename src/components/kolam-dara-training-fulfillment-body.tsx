...
import {
  KOLAM_DARA_TRAINING_FULFILLMENT_CATEGORIES,
...
/** FE `DaraTrainingPage` tab fulfillment (Consent kirim). */
export function KolamDaraTrainingFulfillmentBody({
  canManage,
  refreshKey = 0,
}: {
  canManage: boolean;
  ...
  const [category, setCategory] =
    useState<KolamDaraTrainingPhraseCategory>('fulfillment_grant');
      ...
      setNotice(
        err instanceof ApiError
          ...
          : err instanceof Error
            ? err.message
            : 'Gagal memuat kosa kata consent',
      );
    } finally {
      setLoading(false);
    }
  ...
  const openCreate = (
    initial: 'fulfillment_grant' | 'fulfillment_decline',
  ) => {
    setEditing(null);
    setPhrase('');
    setCategory(initial);
  ...
  const openEdit = (row: KolamDaraTrainingPhrase) => {
    ...
    setCategory(
      KOLAM_DARA_TRAINING_FULFILLMENT_CATEGORIES.includes(row.category)
        ? row.category
        : 'fulfillment_grant',
    );
      ...
      <View style={styles.card}>
        <View style={styles.cardHead}>
          <View style={styles.headCopy}>
            <Text style={styles.sectionTitle}>Kosa kata consent kirim</Text>
            <Text style={styles.meta}>
              Setelah bayar, autopilot menunggu persetujuan customer. Frasa ini
              mengubah status consent — bukan balasan chat. Bisa juga ditambah
              langsung dari Inbox (tombol di balasan consent).
            </Text>
          </View>
          {canManage ? (
            <View style={styles.headActions}>
              <KolamButton
                intent="secondary"
                label="Tahan kirim"
                onPress={() => openCreate('fulfillment_decline')}
                size="sm"
              />
              <KolamButton
                label="Setuju kirim"
                onPress={() => openCreate('fulfillment_grant')}
                size="sm"
              />
            </View>
          ) : null}
        ...
        {loading ? (
          ...
          <Text style={styles.meta}>
            Belum ada kosa kata. Contoh setuju: gas kirim aja · tahan: nanti
            dulu
          </Text>
        ) : (
          <>
            <View style={styles.tableHead}>
              <Text style={[styles.th, styles.colPhrase]}>Frasa customer</Text>
              <Text style={[styles.th, styles.colActionType]}>
                Aksi autopilot
              </Text>
              <Text style={[styles.th, styles.colPriority]}>Prioritas</Text>
            ...
            {rows.map(row => (
              <View key={row.id} style={styles.tableRow}>
                ...
                <View style={styles.colActionType}>
                  <KolamStatusBadge
                    intent={
                      row.category === 'fulfillment_grant'
                        ? 'success'
                        : 'warning'
                    }
                    label={
                ...
                <Text style={[styles.tdMuted, styles.colPriority]}>
        ...
        <View style={styles.modalRoot}>
          ...
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>
              {editing ? 'Edit kosa kata consent' : 'Tambah kosa kata consent'}
            </Text>
            <Text style={styles.meta}>
            ...
            <KolamDetailScrollSurface contentContainerStyle={styles.form}>
              <Field
                ...
                placeholder={
                  category === 'fulfillment_decline'
                    ? 'contoh: nanti dulu'
                    : 'contoh: gas kirim aja'
                }
                value={phrase}
              />
              <KolamDropdownSelect
                label="Aksi autopilot"
                onChange={value =>
                  setCategory(value as KolamDaraTrainingPhraseCategory)
                }
                options={KOLAM_DARA_TRAINING_FULFILLMENT_CATEGORIES.map(id => ({
...
const styles = StyleSheet.create({
  ...
  colPhrase: {flex: 1.5, minWidth: 110},
  colActionType: {flex: 1.2, minWidth: 100},
  colPriority: {flex: 0.7, minWidth: 64},
